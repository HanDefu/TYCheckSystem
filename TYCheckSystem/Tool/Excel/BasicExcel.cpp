...
bool Block::Open(const char* filename, ios_base::openmode mode)
// PURPOSE: Open an existing block file.
// PROMISE: Return true if file is successfully opened, false if otherwise.
{
	// Open existing file for reading or writing or both
	file_.open(filename, mode | ios_base::binary);
	if (!file_.is_open()) return false;
...
bool CompoundFile::Open1(const char* filename, ios_base::openmode mode)
{
	//size_t filenameLength = strlen(filename);
	//wchar_t* wname = new wchar_t[filenameLength+1];
	//mbstowcs(wname, filename, filenameLength);
	//wname[filenameLength] = 0;
	bool ret = Open(filename, mode);
	//delete[] wname;
	return ret;
}
...
bool BasicExcel::Load(const char* filename, MODE mode)
{
	if (file_.IsOpen()) file_.Close();
	...
	if (file_.Open(filename,openMode))
	{
		workbook_ = Workbook();
		worksheets_.clear();
...
std::string BasicExcelCell::Get()
{
	std::string str;
	char output[1028]="";
	if (type_ == WSTRING)
	{
        sprintf(output, "%ws", GetWString() );
		str = std::string(output);
		return str;
	}
	else if (type_ == STRING)
	{
		str = std::string(GetString());
		return str;
	}
	else if (type_ == INT)
	{
		
        sprintf(output, "%d", GetInteger() );
		str = std::string(output);
		return str;
	}
	else if (type_ == DOUBLE)
	{
        sprintf(output, "%.6f", GetDouble() );
		str = std::string(output);
		return str;
	}
	else
